(() => {
    ...
    function initSliders() {
        ...
        if (document.querySelector(".review-block__slider")) new core(".review-block__slider", {
    ...
    da.init();
    function backgroundButton(name, phone, btn) {
        const name1 = document.querySelector(name);
        const phone1 = document.querySelector(phone);
        name1.addEventListener("input", changeBackgraund);
        phone1.addEventListener("input", changeBackgraund);
        function changeBackgraund() {
            if ("" !== name1.value && "" !== phone1.value) {
                document.querySelector(btn).style.transition = "background 10s ease 0s";
                document.querySelector(btn).style.background = "linear-gradient(101.75deg, #E14BE4 -42.13%, #F5D0F5 128.34%)";
            } else {
                document.querySelector(btn).style.transition = "background 10s ease 0s";
                document.querySelector(btn).style.background = "linear-gradient(101.75deg, #999999 -42.13%, #F5F5F5 128.34%)";
            }
        }
    }
    backgroundButton("#name_form", "#phone_form", "#btn_form");
    backgroundButton("#name_pop", "#phone_pop", "#btn_pop");
    function ibg() {
    ...
    function sendMail(i, req) {
        ...
        function formValidate(form) {
            ...
            for (let index = 0; index < formReq.length; index++) {
                ...
                if (input.classList.contains("_phone")) {
                    if (numberTest(input) || "" === input.value) {
                        formAddError(input);
                        error++;
                    }
                } else if ("" === input.value) {
        ...
        function numberTest(input) {
            return !/(^(?!\+.*\(.*\).*\-\-.*$)(?!\+.*\(.*\).*\-$)(([0-9]{0,10})?(\+[0-9]{2,15})?(\([0-9]{2,3})?(\)[0-9]{1})?([-0-9]{8})?([0-9]{0,1})?)$)|(^[0-9]{1,4}$)/.test(input.value);
        }
    }
    sendMail("form", "._req");
    sendMail("form_pop", "._req_pop");
    window["FLS"] = true;
    isWebp();
    spollers();
    pageNavigation();
